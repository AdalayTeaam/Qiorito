...
function obtenerFuenteVideo() {
    return vistaTelefono.matches
        ? "img/phone-scroll.mp4"
        : "img/phone-scroll-escritorio.mp4";
}

function cargarFuenteVideo() {
    ...
    videoListo = false;
    videoScroll.pause();
    videoScroll.src = nuevaFuente;
    videoScroll.load();
}

function aplicarTiempoVideo() {
    if (!videoListo || videoScroll.seeking) return;
    if (Math.abs(videoScroll.currentTime - tiempoObjetivo) < 0.02) return;

    videoScroll.currentTime = tiempoObjetivo;
}
...
videoScroll.addEventListener("loadeddata", function () {
    videoListo = true;
    videoScroll.currentTime = 0.01;
    solicitarActualizacionVideo();
});
